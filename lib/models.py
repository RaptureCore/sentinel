...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, raptured):
        golist = raptured.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_raptured(raptured, item)
    ...
    @classmethod
    def import_gobject_from_raptured(self, raptured, rec):
        import decimal
        import rapturelib
        import inflection
        ...
        # shim/raptured conversion
        object_hex = rapturelib.SHIM_deserialise_from_raptured(object_hex)
        objects = rapturelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from raptured...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from raptured, with every run
        ...
        # get/create, then sync payment amounts, etc. from raptured - Raptured is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(raptured)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from raptured! %s" % e)
            govobj.vote_delete(raptured)
            return (govobj, None)
    ...
    def vote_delete(self, raptured):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(raptured, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, raptured, signal, outcome):
        import rapturelib
        ...
        output = raptured.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = rapturelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(raptured, signal)

    def sync_network_vote(self, raptured, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = raptured.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 rapture addr, non-multisig
            if not rapturelib.is_valid_rapture_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Rapture address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (rapturelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/RaptureDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import rapturelib
        obj_data = rapturelib.SHIM_serialise_for_raptured(self.serialise())
    ...
    def prepare(self, raptured):
        try:
            object_hash = raptured.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not rapturelib.is_valid_rapture_address(addr, config.network):
    ...
    def hash(self):
        import rapturelib
        return rapturelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, raptured):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - raptured.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, raptured):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - raptured.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, raptured):
        now = int(time.time())
        return (self.created_at < (now - raptured.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, raptured):
        if self.is_expired(raptured):
            return False

        return True

    def is_deletable(self, raptured):
        if self.is_expired(raptured):
            return True

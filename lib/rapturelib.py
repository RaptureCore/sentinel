...
def is_valid_rapture_address(address, network='mainnet'):
    ...
    rapture_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Rapture address
        return False

    if (address_version != rapture_version):
        return False
...
# shims 'til we can fix the raptured side
def SHIM_serialise_for_raptured(sentinel_hex):
    ...
    # superblock => "trigger" in raptured
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # raptured expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    raptured_hex = serialise(obj)
    return raptured_hex


# shims 'til we can fix the raptured side
def SHIM_deserialise_from_raptured(raptured_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(raptured_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('rapture.conf')

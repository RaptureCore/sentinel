...
class RaptureDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some raptured methods
    ...
    @classmethod
    def from_rapture_conf(self, rapture_dot_conf):
        from rapture_config import RaptureConfig
        config_text = RaptureConfig.slurp_config_file(rapture_dot_conf)
        creds = RaptureConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = rapturelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = rapturelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = rapturelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_raptured_proto_version_with_sentinel_ping)

...
def has_rapture_conf():
    import config
    import io

    valid_rapture_conf = False

    # ensure rapture_conf exists & readable
    #
    # if not, print a message stating that Rapture Core must be installed and
    # configured, including JSONRPC access in rapture.conf
    try:
        f = io.open(config.rapture_conf)
        valid_rapture_conf = True
    except IOError as e:
        print(e)

    return valid_rapture_conf
...
def main():
    ...
    if not has_rapture_conf():
        print("RaptureCore must be installed and configured, including JSONRPC access in rapture.conf")
        sys.exit(1)

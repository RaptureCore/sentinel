...
def test_valid_rapture_address():
    from rapturelib import is_valid_rapture_address

    main = valid_rapture_address()
    test = valid_rapture_address('testnet')

    assert is_valid_rapture_address(main) is True
    assert is_valid_rapture_address(main, 'mainnet') is True
    assert is_valid_rapture_address(main, 'testnet') is False

    assert is_valid_rapture_address(test) is False
    assert is_valid_rapture_address(test, 'mainnet') is False
    assert is_valid_rapture_address(test, 'testnet') is True


def test_invalid_rapture_address():
    from rapturelib import is_valid_rapture_address

    main = invalid_rapture_address()
    test = invalid_rapture_address('testnet')

    assert is_valid_rapture_address(main) is False
    assert is_valid_rapture_address(main, 'mainnet') is False
    assert is_valid_rapture_address(main, 'testnet') is False

    assert is_valid_rapture_address(test) is False
    assert is_valid_rapture_address(test, 'mainnet') is False
    assert is_valid_rapture_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from rapturelib import elect_mn
...
def test_parse_masternode_status_vin():
    from rapturelib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % rapturelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(rapturelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(rapturelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(rapturelib.blocks_to_seconds(16616)) == 2612035
